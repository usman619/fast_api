...
def vote(vote: schemas.Vote, db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if (vote.vote_dir == 1):
        ...
        db.add(new_vote)
        db.commit()
        return {"message":"Your vote is successfully added."}
    else:
        if not found_vote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Vote with post_id:{vote.post_id} does not exist.")
